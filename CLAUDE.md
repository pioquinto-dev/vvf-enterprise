...
- `MediaArchiver` handles `cover`, `thumbnail_url` and `avatar` as independent attempts, so a dead avatar cannot cost a video its cover. Object keys are `{prefix}/tiktok/{triggerId}_{videoId}_{createdAt}/{triggerId}_{videoId}_{createdAt}_{kind}.{ext}` — deterministic, so each viral video record gets its own folder and a repair overwrites instead of accumulating duplicates.
