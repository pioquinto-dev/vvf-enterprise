...
import { expandKeywords } from '../../landing/flow/api.js';
...
function Stepper({ step }) {
  return <div className="fs-stepper" aria-label={`Step ${step} of 3`}>
    {['Subject', 'Refine', 'Results'].map((label, index) => {
...
export default function Free({ phrase = '', type = 'brand', error = null }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    expandKeywords(subject).then((payload) => {
      if (!active) return;
  ...
  const stashAndGo = async (destination, tag) => {
    ...
    try {
      const response = await fetch('/search/pending', { method: 'POST', credentials: 'same-origin', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrfToken() }, body: JSON.stringify({ type: kind, phrase: subject, keywords: selected, frequency: 'weekly', ...(kind === 'brand' ? { sources: { tiktokHandle: handle.trim().replace(/^@/, ''), website: website.trim() } } : {}) }) });
  ...
  return <><Head title="Free TikTok search · Brand Beacon" /><div className="bbh"><Nav homeHref="/" /><main className={`free-flow ${screen === 'gate' ? 'free-flow--gate' : ''}`}>
    <style>{`
    ...
    `}</style>
    {screen === 'subject' && <section className="ff-subject-form"><h1>What do you want to scan?</h1><form onSubmit={beginRefine}><div className="ff-modes">{TYPES.map(({ key, label, icon: Icon }) => <button key={key} type="button" className={`ff-mode ${kind === key ? 'is-on' : ''}`} onClick={() => setKind(key)}><Icon />{label}</button>)}</div><input className="ff-input" autoFocus value={subject} placeholder={config.placeholder} onChange={(event) => setSubject(event.target.value)} /><button className="ff-run" style={{ marginTop: 16, marginLeft: 'auto' }} disabled={!subject.trim()}>Continue <Arrow /></button></form></section>}
    {screen === 'refine' && <section className="ff-shell"><div className="ff-topbar"><button type="button" className="ff-topbar__back" onClick={() => window.location.assign('/')}>&larr; Back</button><span className="ff-topbar__free">Free search &middot; no card needed</span></div><Stepper step={2} /><div className="ff-card"><div className="ff-subject"><span className="ff-subject__label">Searching</span><strong>{subject}</strong><button type="button" className="ff-edit" aria-label="Change subject" onClick={() => window.location.assign('/')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z" /></svg></button></div><div className="ff-section"><span className="ff-eyebrow">Widen the pull</span><h1>Add keywords to catch more of the trend</h1><p>These are the terms people pair with your subject on TikTok. Ticking more still counts as one free search.</p>{loading ? <p>Suggesting keywords...</p> : <div className="ff-chips">{terms.map((term) => <button key={term.value} type="button" disabled={term.locked} className={`ff-chip ${term.selected ? 'on' : ''}`} onClick={() => toggle(term.value)}><span className="ff-check">{term.selected && <Check />}</span>{term.value}</button>)}{adding ? <input className="ff-add-input" autoFocus value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={addKeyword} onKeyDown={(event) => { if (event.key === 'Enter') addKeyword(); if (event.key === 'Escape') setAdding(false); }} placeholder="Add a keyword" /> : <button type="button" className="ff-chip ff-add" onClick={() => setAdding(true)}><Plus />Add your own</button>}</div>}<p className="ff-count"><b>{selected.length}</b> selected · all covered by your one free search.</p></div>{kind === 'brand' && <div className="ff-section ff-section--sources"><span className="ff-eyebrow">Optional</span><h1>Add your brand&apos;s TikTok handle or website</h1><p>Not required. It helps us match videos more accurately and sharpen the results.</p><label className="ff-source"><span className="ff-source__head"><span><span className="ff-source__ic"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg></span>TikTok handle</span><span>OPTIONAL</span></span><input value={handle} onChange={(event) => setHandle(event.target.value)} placeholder="@yourhandle" /></label><label className="ff-source"><span className="ff-source__head"><span><span className="ff-source__ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="16" rx="2" /><circle cx="9" cy="10" r="2" /><path d="m3 18 6-6 5 5 3-3 4 4" /></svg></span>Website</span><span>OPTIONAL</span></span><input value={website} onChange={(event) => setWebsite(event.target.value)} placeholder="https://yourbrand.com" /></label></div>}<div className="ff-footer"><button type="button" className="ff-back" onClick={() => window.location.assign('/')}>Back</button><button type="button" className="ff-run" disabled={loading || selected.length === 0} onClick={() => setScreen('gate')}>Run my free search <Arrow /></button></div></div></section>}
